...
//                                                    //
//****************************************************//
void llenar_matriz_identidad(float **i, int t); 
...
float** crear_matriz(int t)
{
  ...
  m = new float *[t];
  for(int i=0; i<t; i++)
    m[i] = new float[t+1]; // t+1 es para agregar el vector resultado a la matriz
  return m;
}
...
void imprimir_matriz(float **m, int t)
{
  ...
  for(int i=0; i<t; i++)
  {
    std::cout << std::string(6*(t+1) + 3*(t+1), '-') << "\n";
    std::cout << "|";
    for(int j=0; j<t+1; j++)
      std::cout << prd(m[i][j],2,6)  << " | ";
    cout << endl;
  }
  std::cout << std::string(6*(t+1) + 3*(t+1), '-') << endl;
...
void to_zero( float **m, int col, int size )
{
  ...
  //float result;
  float product;
  int op = 0; // Operacion: 1 = suma; 2 = reat; 0 = sin operacion asignada
  ...
  for( int x=0; x<size; x++ )  // Recorrer la columna
  {
    ...
    if( x != col )
    {
      ...
      {
        product = m[col][y] * value;
        if(op==0)
        {
          if( (m[x][y]>0 && product>0) || (m[x][y]<0 && product<0) ) //signos iguales se restan
          {
            op=2; // restar
            cout << "Fila " << x << " - fila " << col << " * " << value << endl;
          }
          else if( (m[x][y]>0 && product<0) || (m[x][y]<0 && product>0) ) //signos diferentes se suman
          {
            op = 1; //sumar
            cout << "Fila " << x << " + fila " << col << " *  " << value << endl;
          }
